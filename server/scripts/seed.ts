...
import { getFragellaClient, isFragellaConfigured } from '../lib/fragella'
import { normalizeFragrances } from '../lib/normalize'
import { addProducts, getDbStats } from '../db/store'

interface SeedConfig {
  brands: string[]
  maxItemsPerBrand: number
  ...
  try {
    for (const brand of SEED_CONFIG.brands) {
      if (totalRequests >= SEED_CONFIG.maxTotalRequests) {
        console.log(
          `⚠️  Reached request limit (${SEED_CONFIG.maxTotalRequests}). Stopping to preserve API quota.`,
        )
        break
      }
      ...
      try {
        ...
        const normalized = normalizeFragrances(fragrances)
        const added = addProducts(normalized)

        console.log(
          `   ✅ ${added}/${fragrances.length} fragrances added (${fragrances.length - added} duplicates)`,
        )
        totalAdded += added
        totalRequests++
      } catch (error) {
